...
import { Puzzle, Session } from "./types.js";
...
(async () => {
    ...
    const currentChallenge: HTMLElement = document.getElementById("current_puzzle") as HTMLElement;
    ...
    currentPoints.innerText = `0 / ${puzzle.score.toString()}`;
    
    // Description is an image
    // Get the image
    const fileRes: Response = await sendGet(`puzzles/${problemId}/file`, { "Authorization": getToken() });
    if (fileRes.status === 404) {
        (<HTMLImageElement>document.getElementById("puzzle-image")).src = "/images/University_of_Twente_panoramic.jpg";
    } else {
        const reader: FileReader = new FileReader();
        reader.onload = (e) => {
            (<HTMLImageElement>document.getElementById("puzzle-image")).src = e.target.result.toString();
        }
    ...
    if (submissionRes.status === 200) {
        ...
        // document.getElementById("descriptionAccordionBody").ariaExpanded = "true";
